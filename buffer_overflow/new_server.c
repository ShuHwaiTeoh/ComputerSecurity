...
/ from the client.c program. where 8000 is the port you want your server to monitor.
*/
...
char * clientComm(int clntSockfd,int * senderBuffSize_addr, int * optlen_addr){
    ...
    // if the number of received byte is larger than MAX_DATA_SIZE
    // avoid using strcpy for variable str.
    while(numBytes >= MAX_DATA_SIZE){
        fprintf(stderr,"Too many characters for input will cause buffer overflow.\n");

        //send "less" to let client input less characters
